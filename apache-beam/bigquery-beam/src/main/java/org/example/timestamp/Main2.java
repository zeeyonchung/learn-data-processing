 ...
 *
 * 실행 명령어:
 * mvn compile exec:java -Dexec.mainClass=org.example.timestamp.Main2 \
...
public class Main2 {
    ...
    private static class Destination extends DynamicDestinations<TableRow, String> {
        @Override
        public String getDestination(ValueInSingleWindow<TableRow> element) {
            return "project_id.sample_dataset.sample_table";
        }
        ...
        public TableSchema getSchema(String destination) {
            return new TableSchema().setFields(Arrays.asList(
                    new TableFieldSchema().setName("field3").setType("DATETIME"),
                    new TableFieldSchema().setName("field4").setType("TIMESTAMP")
            ));
        }
    }
}
